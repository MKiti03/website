...
def index(request):
    ...
    featured_coutries = Country.objects.all().filter(set_draft = False).order_by('country_name')[:6]
    
    # featured universities on  the home page
    featured_universities = University.objects.all().filter(set_draft = False).order_by('university_name')[:8]
